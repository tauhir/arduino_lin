...
int lin_stack::setSerial(){ // Only needed when receiving signals
	if(ch==1){ // For LIN1 (Channel 1)
		LINBusSerialOne.begin(bound_rate); // Configure Serial1
	} else if(ch==2){ // For LIN2 (Channel 2)
		LINBusSerialTwo.begin(bound_rate); // Configure Serial2
	}
}

int lin_stack::read(byte data[], byte data_size, boolean all_data = false){
